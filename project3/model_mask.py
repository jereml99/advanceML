...
class SimpleGraphConv(torch.nn.Module):
    ...
    def __init__(self, node_feature_dim, filter_length, M=1):
        ...
        self.output_net = torch.nn.Sequential(
            torch.nn.Linear(self.node_feature_dim, 8),
            torch.nn.ReLU(),
            torch.nn.Linear(8, 16),
            torch.nn.ReLU(),
            torch.nn.Linear(16, 8),
            torch.nn.ReLU(),
            torch.nn.Linear(8, M * 2),
        )
...
class BernoulliDecoder(torch.nn.Module):
    ...
    def forward(self, z, mask = None):
        ...
        probs_masked = probs * mask_matrix
        return td.Independent(td.Bernoulli(probs=probs_masked), 2)
...
class VAE(L.LightningModule):
    ...
    def elbo(self, X, A):
        ...
        elbo = torch.mean(
            self.decoder(z, mask).log_prob(A) - td.kl_divergence(q, self.prior()), dim=0
        )
        return elbo
