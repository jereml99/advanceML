...
class baseline():
    def __init__(self, training_samples: List[nx.graph.Graph] ) -> None:
        ...
        for training_sample in training_samples:
             self.edge_number_distribution_per_number_of_nodes[training_sample.number_of_nodes()].append(training_sample.number_of_edges())
    
    def sample(self, number_of_samples = 1) -> List[nx.graph.Graph]:
        node_numbers = np.random.choice(self.node_number_distribution, number_of_samples, replace=True)
        samples = []
