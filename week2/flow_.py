...
class MaskedCouplingLayer(nn.Module):
    ...
    def forward(self, z):
        ...
        """
        # z is the input to the coupling layer
        # Apply mask to input
        z_masked = self.mask * z
        # Compute scale and translation
        s = self.scale_net(z_masked)
        t = self.translation_net(z_masked)
        # Apply transformation to masked elements
        z_transformed = (1 - self.mask) * (z * torch.exp(s) + t)
        # Combine unchanged and transformed elements
        z_prime = z_masked + z_transformed
...
if __name__ == "__main__":
    ...
    train_loader = torch.utils.data.DataLoader(
        minist_dataset, batch_size=args.batch_size, shuffle=True
    )
