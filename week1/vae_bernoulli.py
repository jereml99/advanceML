...
class GaussianEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        mean, std = torch.chunk(self.encoder_net(x), 2, dim=-1) # It's for spliting the encoder output into mean and std
        return td.Independent(td.Normal(loc=mean, scale=torch.exp(std)), 1) # It creating one multiy dimensional gaussian distribution out of X (latent space size) distribution
...
def plot(model, data_loader, device):
    ...
    for x, y in data_loader:  # Assuming the data_loader provides the input data and labels
        x = x.to(device)
        with torch.no_grad():
            q = model.encoder(x)
            z = q.rsample()
    ...
    for i in np.unique(labels):
        indices = labels == i
        plt.scatter(latent_variables_2d[indices, 0], latent_variables_2d[indices, 1], label=f'Class {i}')
